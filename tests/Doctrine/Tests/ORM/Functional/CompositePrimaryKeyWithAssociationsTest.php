...
class CompositePrimaryKeyWithAssociationsTest extends \Doctrine\Tests\OrmFunctionalTestCase
{
    public function setUp()
    {
        ...
        $admin1->addName($name1);
        $admin1->addName($name2);
    ...
    public function testFindByAbleToGetCompositeEntitiesWithMixedTypeIdentifiers()
    {
        ...
        $this->assertEquals(1, $name1->getId());
        $this->assertEquals("Roma", $name1->getName());

        $this->assertEquals(2, $name2->getId());
        $this->assertEquals("Rome", $name2->getName());
    }
